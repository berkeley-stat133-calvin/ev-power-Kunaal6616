...
# Check if files are in a different directory (like Downloads or a project folder)
file_list_downloads <- list.files("~/Downloads", pattern = "\\.csv$")
print("CSV files in Downloads:")
print(file_list_downloads)

# If files are in Downloads, set working directory or use full paths
...
if(loaded_status["renew_2023"] && loaded_status["energy_use_2023"]) {
  ...
  # Calculate renewable percentage for 2023
  ...
  # Build analysis dataset step by step
  ...
  # Add energy price data if available (filter for 2023 if year column exists)
  ...
  # If we don't have the main datasets, create minimal analysis data
  print("Creating minimal analysis dataset from available data...")
  
  # Try to use whatever renewable data we have
...
# Create mapping data (remove any rows with NA in key columns)
...
# Calculate basic statistics with the shown data
...
# Prepare mapping data with proper state names
mapping_data_clean <- mapping_data

# If we have renewable percentage data, categorize it
if("renew_percent" %in% names(mapping_data_clean)) {
  ...
  # If we only have total renewable, create a simple classification
  mapping_data_clean <- mapping_data_clean %>%
    dplyr::mutate(
      state_lower = tolower(state),
      renew_category = dplyr::case_when(
...
# Create static map - Renewable Energy by State
...
# Create EV registration map if we have the data
...
# Create scatter plot: Renewable % vs EV Registrations
if("renew_percent" %in% names(analysis_data) && "ev_registrations" %in% names(analysis_data)) {
